...
void setupTimers(void) {
  ...
  // Set high priority for pulse end (must be fast for accurate pulse width)
  HAL_NVIC_SetPriority(TIM4_IRQn, 2, 0);  // Higher priority than ignition delay
...
void sendRealtimeData(void) {
  // Format: RT:RPM,TIMING,TEMP,BATT,CHARGING,MAP,LIMITER,FLAGS,PEAK,CPU,RAM,TRIG,CUT,ENGTYPE,CFGSRC,QSADC,TEMPRAW,BATTRAW,CHRGRAW
  ...
  int len = snprintf(rtBuffer, sizeof(rtBuffer),
    "RT:%u,%d,%d,%d,%d,%u,%u,%u,%u,%u,%u,%d,%u,%u,%u,%u,%u,%u,%u\n",
    runtime.currentRpm,
